...
static void rockchip_i2s_tdm_xfer_trcm_start(struct rk_i2s_tdm_dev *i2s_tdm, int stream)
{
...
//        ret = regcache_sync(i2s_tdm->regmap);
//        if (ret)
//            dev_err(i2s_tdm->dev, "rockchip_i2s_tdm_xfer_trcm_start regmap err\n");

	spin_unlock_irqrestore(&i2s_tdm->lock, flags);
}

static void rockchip_i2s_tdm_xfer_trcm_stop(struct rk_i2s_tdm_dev *i2s_tdm, int stream)
{
	unsigned long flags;
...
//        regcache_cache_only(i2s_tdm->regmap, true); // rc
	spin_unlock_irqrestore(&i2s_tdm->lock, flags);
}
